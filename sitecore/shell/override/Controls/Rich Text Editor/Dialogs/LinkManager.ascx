...
<script type="text/javascript">
	...
	Telerik.Web.UI.Widgets.LinkManager.prototype = {
		...
		_loadLinkProperties: function ()
		{
			...
			this._linkAriaLabel.value = currentLink.getAttribute("aria-label", "");
		...
		getModifiedLink: function ()
		{
			...
			if (selectedTabValue == "HyperlinkTab")//"link"
			{
				...
				if (this._linkAriaLabel.value.trim() == "")
				{
					resultLink.removeAttribute("aria-label", 0);
				}
				else
				{
					resultLink.ariaLabel = this._linkAriaLabel.value;
				}

				// XAContext customization
	            resultLink.removeAttribute("name", 0);
				var linkCssClass = this._linkCssClass.get_value();
			...
			else //"email"
			{
				...
				this._setClass(resultLink, this._emailCssClass.get_value());
			}

			return resultLink;
		},

		_setClass: function (element, cssClass)
		{
			if (cssClass == "")
			{
		...
		setupChildren: function ()
		{
			...
			this._linkAriaLabel = $get("LinkAriaLabel");
			...
			<telerik:RadMultiPage ID="dialogMultiPage" runat="server" SelectedIndex="0">
				<telerik:RadPageView ID="hyperlinkFieldset" runat="server">
					<table border="0" cellpadding="0" cellspacing="0" class="reControlsLayout">
						...
						<tr>
							<td class="reLabelCell">
								<label for="LinkAriaLabel" class="reDialogLabel">
									<span>
										Aria Label
									</span>
								</label>
							...
							<td class="reControlCell">
								<input type="text" id="LinkAriaLabel" />
							</td>
						</tr>
					</table>
				</telerik:RadPageView>
